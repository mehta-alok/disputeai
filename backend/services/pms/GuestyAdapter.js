  ...
  async pushFlag(guestId, flagData) {
    ...
    const { result, durationMs } = await this._timed(async () => {
      const response = await this.httpClient.put(
        `/guests/${encodeURIComponent(guestId)}`,
        {
          tags: ['DisputeAI-Flag', `severity-${flagData.severity || 'high'}`, 'chargeback-risk'],
          notes: {
            title: `DisputeAI Flag: ${flagData.reason}`,
            body: [
              ...
              'Source: DisputeAI Chargeback Defense',
            ].filter(Boolean).join('\n'),
          },
        }
      );
  ...
  async registerWebhook(config) {
    ...
    const { result, durationMs } = await this._timed(async () => {
      const response = await this.httpClient.post('/webhooks', {
        ...
        secret,
        active: true,
        description: 'DisputeAI Chargeback Defense Integration',
      });
      return response.data;
    });
