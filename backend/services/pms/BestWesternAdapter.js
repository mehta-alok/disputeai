...
class BestWesternAdapter extends BasePMSAdapter {
   ...
   * @param {string} [config.credentials.partnerId]      - DisputeAI partner ID in BW system.
  ...
  constructor(config) {
    ...
    this.partnerId = this.credentials.partnerId || 'DISPUTEAI';
  }
  ...
  async pushNote(guestId, note) {
    ...
    const bwNote = {
      note: {
        ...
        title: note.title,
        body: `[${note.category || 'DisputeAI'}] ${note.title}\n\n${note.content}`,
  ...
  async pushFlag(guestId, flagData) {
    ...
    const bwAlert = {
      alert: {
        ...
        title: `DisputeAI Flag: ${flagData.severity?.toUpperCase() || 'HIGH'}`,
    ...
    const bwComment = {
      comment: {
        ...
        body: [
          ...
          '---',
          'Generated by DisputeAI Chargeback Defense System',
        ].join('\n'),
