...
class MewsAdapter extends BasePMSAdapter {
   ...
   * @param {string} [config.credentials.client]     - Integration client name (e.g. "DisputeAI").
  ...
  constructor(config) {
    ...
    this.clientName = this.credentials.client || 'DisputeAI';
  ...
  async pushNote(guestId, note) {
    ...
    const mewsNote = this._baseBody({
      CustomerId: guestId,
      Notes: `[${note.category || 'DisputeAI'}] ${note.title}\n${note.content}`,
    });
    ...
    const { result, durationMs } = await this._timed(async () => {
      ...
      const response = await this.httpClient.post('/customers/update', {
        ...
        CustomerUpdates: [{
          CustomerId: guestId,
          Notes: {
            Value: `[${note.category || 'DisputeAI'}] ${note.title}\n${note.content}`,
          },
    ...
    const { result, durationMs } = await this._timed(async () => {
      const response = await this.httpClient.post('/customers/update', {
        ...
        CustomerUpdates: [{
          ...
          Notes: {
            Value: `DISPUTEAI FLAG: ${flagData.reason}` +
